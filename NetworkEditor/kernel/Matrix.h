...
class MatrixPool{
private:
	vector<shared_ptr<AbstractMatrix>> pool;
	static shared_ptr<MatrixPool> instance;
	MatrixPool();
public:
	static shared_ptr<MatrixPool> getInstance();
	shared_ptr<AbstractMatrix> allocMatrix(size_t rowSize, size_t columnSize);
    shared_ptr<AbstractMatrix> allocMatrixUnClean(size_t rowSize, size_t columnSize);
};
...
public:
	...
	double sum() override;
	double max() override;
	void setAllValue(double value)override;
	...
	shared_ptr<AbstractMatrix> add_inplace(size_t, size_t, size_t, size_t, 
		shared_ptr<AbstractMatrix>, size_t, size_t, size_t, size_t, double) override;
