...
shared_ptr<CharacterSample> XMLDataParser::getNextCharacterSample(){
	...
	//if condition is not training, read output to sample
...
shared_ptr<TrainingSample> XMLDataParser::getNextTrainingSample(){
	if (this->sampleIndex >= sampleNum){
		this->sampleIndex = 0;
		TiXmlElement *root = (doc->RootElement());
		TiXmlElement *samples = (root->FirstChildElement("samples"));
	...
	TiXmlElement *inputElement = (this->sample->FirstChildElement("input"));
	string inputStr = inputElement->GetText();
	shared_ptr<TrainingSample> trainingSample;
	...
	//input->print();
	trainingSample = shared_ptr<TrainingSample>(new TrainingSample(input, output, type, iRowSize, iColumnSize, oRowSize, oColumnSize));
	TiXmlElement *releaseSample = sample;
	sample = (sample->NextSiblingElement());
	this->sampleIndex++;
...
shared_ptr<Sample> XMLDataParser::getNextSample(){
	...
	//input->print();
	trainingSample = shared_ptr<Sample>(new Sample(input,type, iRowSize, iColumnSize, oRowSize, oColumnSize));
	TiXmlElement *releaseSample = sample;
	sample = (sample->NextSiblingElement());
	this->sampleIndex++;
	//trainingSample->getInput()->print();
