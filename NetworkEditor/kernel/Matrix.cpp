...
inline shared_ptr<AbstractMatrix> MatrixPool::allocMatrix(size_t rowSize, size_t columnSize){
	for (int i = pool.size()-1; i >=0; i--){
		if (pool[i].use_count() == 1 && pool[i]->getRowSize() == rowSize && pool[i]->getColumnSize() == columnSize){
			//cout << "in" << endl;
			pool[i]->setAllValue(0);
			return pool[i];
		}
	}
	shared_ptr<AbstractMatrix> newMatrix(new Matrix(rowSize, columnSize));
	pool.push_back(newMatrix);
	return newMatrix;
}
inline shared_ptr<AbstractMatrix> MatrixPool::allocMatrixUnClean(size_t rowSize, size_t columnSize){
    for (int i = pool.size() - 1; i >= 0; i--){
        if (pool[i].use_count() == 1 && pool[i]->getRowSize() == rowSize && pool[i]->getColumnSize() == columnSize){
            //cout << "in" << endl;
            return pool[i];
...
/*int main(){
	shared_ptr<MatrixPool> temp = MatrixPool::getInstance();
	shared_ptr<AbstractMatrix> m1=temp->allocMatrix(1, 2);
	cout << m1.use_count() << endl;
	m1->multiple(1);
	cout << m1.use_count() << endl;
	while (1);
}*/
...
Matrix::~Matrix()
{
    ...
    delete[] realMatrixBlock;
	delete[] matrix;
}

void Matrix::initialize()
{
    realMatrixBlock = new double[rowSize*columnSize];

	matrix = new double *[rowSize];

	for (size_t i = 0; i < rowSize; i++)
	{
		matrix[i] = &realMatrixBlock[i*columnSize];
...
void Matrix::setAllValue(double value){
	if (value == 0){
        /*
		for (int i = 0; i < rowSize; i++){
			memset(matrix[i], 0, columnSize*sizeof(double));
        }*/
        memset(realMatrixBlock, 0, sizeof(double)*columnSize*rowSize);
	}
	else{
		for (size_t i = 0; i < rowSize; i++)
		{
...
shared_ptr<AbstractMatrix> Matrix::transpose()
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrix(columnSize, rowSize);
...
shared_ptr<AbstractMatrix> Matrix::submatrixShrink(int t, int b, int l, int r)
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrixUnClean(bottom-top, right-left);
...
shared_ptr<AbstractMatrix> Matrix::submatrixExpand(int top, int bottom, int left, int right)
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrix(bottom - top, right - left); 
	...
	shared_ptr<AbstractMatrix> right_bottom)
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrix(rowSize + left_bottom->getRowSize(), columnSize + right_top->getColumnSize());
...
shared_ptr<AbstractMatrix> Matrix::mergeRow(shared_ptr<AbstractMatrix> bottom)
{
	...
	return merge(MatrixPool::getInstance()->allocMatrix(rowSize, 0), bottom,
		MatrixPool::getInstance()->allocMatrix(bottom->getRowSize(), 0));
...
shared_ptr<AbstractMatrix> Matrix::mergeColumn(shared_ptr<AbstractMatrix> right)
{
	...
	return merge(right, MatrixPool::getInstance()->allocMatrix(0, columnSize),
		MatrixPool::getInstance()->allocMatrix(0, right->getRowSize()));
...
shared_ptr<AbstractMatrix> Matrix::add(double x)
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrix(rowSize, columnSize);
...
shared_ptr<AbstractMatrix> Matrix::multiple(double x)
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrixUnClean(rowSize, columnSize);
    double **mArray = ((Matrix*)(result.get()))->matrix;
	for (size_t i = 0; i < rowSize; i++)
	{
		for (size_t j = 0; j < columnSize; j++)
		{
            mArray[i][j] = matrix[i][j] * x;
			//result->setValue(i, j, matrix[i][j] * x);
		}
	}
...
shared_ptr<AbstractMatrix> Matrix::multipleLeft(shared_ptr<AbstractMatrix> m)
{
	...
	if (rowSize < 4 || columnSize < 4 || m->getRowSize() < 4) {
		...
		result = MatrixPool::getInstance()->allocMatrix(m->getRowSize(), columnSize);
...
shared_ptr<AbstractMatrix>Matrix::map(double func(double))
{
	...
	shared_ptr<AbstractMatrix> result = MatrixPool::getInstance()->allocMatrix(this->rowSize, this->columnSize);
	for (size_t i = 0; i < rowSize; i++)
	{
		for (size_t j = 0; j < columnSize; j++) {
...
double Matrix::convolve(size_t top, size_t bottom, size_t left, size_t right, shared_ptr<AbstractMatrix>m)
{
	...
	/*if (!bounded(top, left) || !bounded(bottom - 1, right - 1)) {
		cerr << "Matrix::covolve Out of Bound" << endl;
		return 0;
	}
	if ((bottom - top != m->getRowSize()) || (right - left != m->getColumnSize())) {
		cerr << "Size mismatched!" << endl;
		return 0;
	}*/
...
shared_ptr<AbstractMatrix>Matrix::m2vByColumn()
{
	...
	shared_ptr<AbstractMatrix> m = MatrixPool::getInstance()->allocMatrix(unit, 1);
...
shared_ptr<AbstractMatrix>Matrix::v2mByColomn(size_t size)
{
	...
	shared_ptr<AbstractMatrix> m = MatrixPool::getInstance()->allocMatrix(rowSize, columnSize);
	for (size_t i = 0; i < rowSize; i++)
	{
		for (size_t j = 0; j < columnSize; j++)
		{
...
shared_ptr<AbstractMatrix> Matrix::add_inplace(shared_ptr<AbstractMatrix> m)
{
	...
	for (size_t i = 0; i < rowSize; i++)
	{
		for (size_t j = 0; j < columnSize; j++)
		{
			//this->setValue(i, j, this->matrix[i][j] + m->getValue(i, j));
            this->matrix[i][j] += m->getValue(i, j);
...
shared_ptr<AbstractMatrix> Matrix::add_inplace(size_t top, size_t bottom, size_t left, size_t right, shared_ptr<AbstractMatrix> m)
{
    ...
    size_t rowSize = m->getRowSize();
    size_t columnSize = m->getColumnSize();
    //Matrix *mImpl = dynamic_cast<Matrix *>(m.get());
	for (size_t i = 0, ii = top; i < rowSize; i++, ii++)
	{
		for (size_t j = 0, jj = left; j < columnSize; j++, jj++)
		{
            this->matrix[ii][jj] += m->getValue(i, j);
...
shared_ptr<AbstractMatrix> Matrix::add_inplace(size_t top, size_t bottom, size_t left, size_t right, 
	shared_ptr<AbstractMatrix> m, size_t top_m, size_t bottom_m, size_t left_m, size_t right_m, double val)
{
	for (size_t i = top, ii = top_m; i < bottom; i++, ii++)
	{
		for (size_t j = left, jj = left_m; j < right; j++, jj++)
		{
			matrix[i][j] += m->getValue(ii, jj) * val;
		}
	}
