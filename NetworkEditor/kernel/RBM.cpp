...
RBM::RBM(size_t visualUnit, size_t hiddenUnit, size_t nonLinearType, bool sparse, size_t initScheme)
{
	...
	this->biasH2VGradient->setAllValue(0);
...
double RBM::singleTraining(shared_ptr<AbstractMatrix> m)
{
	...
	*/
	...
	weightGradient->add_inplace(diff);
	biasGradient->add_inplace(h[0]->subtract(h[sampleLength - 1]));
	biasH2VGradient->add_inplace(v[0]->subtract(v[sampleLength - 1]));
...
void RBM::selfUpdate(bool regularize)
{
	...
	weight->add_inplace(weightGradient->multiple_inplace(weightLearningRate));
	bias->add_inplace(biasGradient->multiple_inplace(biasLearningRate));
	biasH2V->add_inplace(biasH2VGradient->multiple_inplace(biasLearningRate));

	weightGradient->setAllValue(0);
	biasGradient->setAllValue(0);
	biasH2VGradient->setAllValue(0);
...
void RBM::writeSelf(string filename)
{
	...
	fout << nonLinearType << endl;
	fout << sparse << endl;

	fout << selfBatchSize << endl;
	fout << selfTrainingTime << endl;
	fout << sampleLength << endl;
